var jpql_macros = require('./jpql-macros');
...
var grammar = {
  ...
  lex: {
    ...
    macros: {
      esc: "\\\\",
      /** new macros from https://github.com/cjihrig/jsparser/blob/master/ecmascript.jison */
      DecimalDigit:                     "[0-9]",
      DecimalDigits:					"[0-9]+",
      NonZeroDigit:					"[1-9]",
      OctalDigit:					"[0-7]",
      ...
      /** new macros from https://github.com/cjihrig/jsparser/blob/master/ecmascript.jison */
      ReservedKeywords: jpql_macros.tokens.reservedWords.reservedKeywords,
      ScriptExpressionToken: "\\(.+?\\)(?=[\\:\\,\\[\\]])",
      ActiveScriptExpressionToken: "\\{(.+?)\\}(?=[\\:\\,\\[\\]])",
      FilterExpressionToken: "\\?\\(.+?\\)(?=[\\:\\,\\[\\]])"
    },
    startConditions: { INITIAL: "// Default initial Jison/Lex context", IdentifierName: "// IdentifierName" },

    rules: [
      ["\\$(?![\\$a-zA-Z0-9_])", "return 'DOLLAR'"],
      ["\\.\\.", "return 'DOT_DOT'"],
      ["\\.", "return 'DOT'"],
      ["\\*", "return 'STAR_TOKEN'"],
      [jpql_macros.tokens.reservedWords[true], "return 'TRUE_TOKEN'"], //true followed by !IdentifierPart
      [jpql_macros.tokens.reservedWords[false], "return 'FALSE_TOKEN'"],
      [jpql_macros.tokens.reservedWords[undefined], "return 'UNDEFINED_TOKEN'"],
      [jpql_macros.tokens.reservedWords[null], "return 'NULL_TOKEN'"],
//      [jpql_macros.identifier, "return 'IDENTIFIER_NAME'"],
      ["{ReservedKeywords}", "throw 'Illegal keyword: ' + yytext"],
      ["{Identifier}", "return 'IDENTIFIER_NAME'"],
      ["\\[", "return '['"],
      ["\\]", "return ']'"],
      [",", "return ','"],
      ["({DecimalIntegerLiteral})?\\:({DecimalIntegerLiteral})?(\\:({DecimalIntegerLiteral})?)?", "return 'ARRAY_SLICE'"],
      ["{ActiveScriptExpressionToken}?\\:{ActiveScriptExpressionToken}?(\\:{ActiveScriptExpressionToken}?)?", "yytext = [ yy.lexer.matches[1], yy.lexer.matches[3], yy.lexer.matches[5] || 1]; return 'ACTIVE_SLICE'"],
      ["{ActiveScriptExpressionToken}", "return 'ACTIVE_SCRIPT_EXPRESSION_TOKEN'"],
      ["{DecimalIntegerLiteral}", "return 'INTEGER'"],
      [jpql_macros.qq_string, "yytext = yytext.substr(1,yyleng-2); return 'QQ_STRING';"],
      [jpql_macros.q_string, "yytext = yytext.substr(1,yyleng-2); return 'Q_STRING';"],
      ["{ScriptExpressionToken}", "return 'SCRIPT_EXPRESSION_TOKEN'"],
      ["{FilterExpressionToken}", "return 'FILTER_EXPRESSION_TOKEN'"]
    ]
  },
  ...
  bnf: {
    ...
    CHILD_MEMBER_COMPONENT: [
      [ 'DOT MEMBER_EXPRESSION', '$$ = $2' ] ],

    LEADING_CHILD_MEMBER_EXPRESSION: [
      [ 'MEMBER_EXPRESSION', 'yy.ast.set({ scope: "child", operation: "member" })' ] ],

    DESCENDANT_MEMBER_COMPONENT: [
      [ 'DOT_DOT MEMBER_EXPRESSION', '$$ = $2' ] ],
    ...
    SUBSCRIPT_COMPONENT: [
      [ 'CHILD_SUBSCRIPT_COMPONENT',      '$$ = yy.ast.merge($1, { scope: "child" })' ], // branch hasn't been popped yet
      [ 'DESCENDANT_SUBSCRIPT_COMPONENT', '$$ = yy.ast.merge($1, { scope: "descendant" })' ] ],

    DESCENDANT_SUBSCRIPT_COMPONENT: [
      [ 'DOT_DOT CHILD_SUBSCRIPT_COMPONENT', '$$ = $2' ] ],
    ...
    SUBSCRIPT: [
//      [ 'SUBSCRIPT_EXPRESSION_NON_LISTABLE', '$$ = $1; yy.ast.set($$)' ],
...
//     [ 'END', '' ] ],
    ...
    SUBSCRIPT_ACTIVE_EXPRESSION_LISTABLE: [
      ...
      [ 'ACTIVE_SCRIPT_EXPRESSION_TOKEN',       '$$ = { expression: { type: "script_expression|active", value: $1 } }; yy.ast.node($$)' ],
      [ 'STAR', '$$ = $1' ],
      [ 'SCRIPT_EXPRESSION', '$$ = $1' ],
      [ 'FILTER_EXPRESSION', '$$ = $1' ],
      [ 'DOLLAR',                 '$$ = { expression: { type: "root", value: "$$" } }; yy.ast.node($$)' ] ],

    SUBSCRIPT_COMPONENT_EXPRESSION_LISTABLE: [
      [ 'SUBSCRIPT_EXPRESSION_NESTABLE', '$$ = $1'] ],

    /** After setting the composite attributes that as a member of a list the nested expression might miss out on by not making it all the way up to --> CHILD_SUBSCRIPT_COMPONENT -> SUBSCRIPT_COMPONENT, the nested subscription expression is done and ready to go */
    SUBSCRIPT_EXPRESSION_NESTABLE: [
      ...
      [ 'DESCENDANT_MEMBER_COMPONENT NESTED_PATH_COMPONENTS', '$$ = yy.ast.rollIntoParent(yy.ast.merge($1, { scope: "descendant", operation: "member" }), yy.ast.stashPop()); yy.ast.pop()' ],
      [ 'SUBSCRIPT_COMPONENT NESTED_PATH_COMPONENTS', '$$ = yy.ast.rollIntoParent({ expression: { type: "active_position", value: "{index}" } }, [ yy.ast.merge($1, { operation: "subscript" }) ].concat(yy.ast.stashPop())); yy.ast.pop()' ],
      [ 'CHILD_MEMBER_COMPONENT NESTED_PATH_COMPONENTS', '$$ = yy.ast.rollIntoParent({ expression: { type: "active_position", value: "{index}" } }, [ yy.ast.merge($1, { scope: "child", operation: "member" }) ].concat(yy.ast.stashPop())); yy.ast.pop()' ],
      [ 'SUBSCRIPT_COMPONENT', '$$ = yy.ast.rollIntoParent({ expression: { type: "active_position", value: "{index}" } }, [ yy.ast.merge($1, { operation: "subscript" }) ]); yy.ast.pop()' ], // subscript didn't start a branch
      [ 'CHILD_MEMBER_COMPONENT', '$$ = yy.ast.rollIntoParent({ expression: { type: "active_position", value: "{index}" } }, [ yy.ast.merge($1, { scope: "child", operation: "member" }) ]); yy.ast.pop()' ] //child member didn't start a branch
    ],
    ...
    ReservedWord: [
      ...
      [ 'UNDEFINED_TOKEN', ' $$ = { expression: { type: "keyword", value: null } }; yy.ast.node($$)' ] /** undefined upsets the logic and removes the expression.value */
    ],
