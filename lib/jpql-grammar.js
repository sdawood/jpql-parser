...
var grammar = {
  ...
  lex: {
    ...
    macros: {
      ...
      ActiveScriptExpressionToken: "\\({Take}?{Operation}?(\\{.+?\\})?\\)(?=[\\:\\[\\]\\,\\{\\}\\.]|$)",
...
//      [jpql_macros.identifier, "return 'IDENTIFIER_NAME'"],
      ...
      ["{ActiveScriptExpressionToken}(\\:{ActiveScriptExpressionToken})?", "yytext = " +
      "{" +
        "value: yy.lexer.matches[0]," +
        "map: {" +
          "value: yy.lexer.matches[1]," +
          "async: yy.lexer.matches[3]," +
          "take: yy.lexer.matches[5]," +
          "operation: yy.lexer.matches[10]," +
          "script: yy.lexer.matches[11]" +
        "}," +
        "reduce: {" +
            "value: yy.lexer.matches[13]," +
            "async: yy.lexer.matches[15]," +
            "take: yy.lexer.matches[17]," +
            "operation: yy.lexer.matches[22]," +
            "script: yy.lexer.matches[23]" +
        "}" +
      "}" +
