...
var grammar = {
  ...
  lex: {
    ...
    macros: {
      ...
      /*@todo: TIME TO MOVE TO JISON GRAMMAR FILE SYNTAX*/
      RegularExpressionNonTerminator: "[^\\n\\r]",
      RegularExpressionBackslashSequence: "\\\\{RegularExpressionNonTerminator}",
      RegularExpressionClassChar: "[^\\n\\r\\]\\]|{RegularExpressionBackslashSequence}",
      RegularExpressionClass: "\\[{RegularExpressionClassChar}*\\]",
      RegularExpressionFlags: "\\b(?!(?:.\\B)*(.)(?:\\B.)*\\1)[gim]?\\b", // original from http://stackoverflow.com/questions/13546187/how-to-match-any-combination-of-letters-using-regex
      ...
      ScriptOperation: "[~=\\+&\\-\\*]*", //map, (post or put), put, post, merge, delete, splat
      Flags: "=|!|<=|<!", //
      Provides: "=>", // tazbits source
      ...
      ActiveFilterExpressionToken: "\\?(?!\\(){Flags}?\\s*{Tag}?\\s*{AsyncTake}?\\s*{ScriptOperation}?\\s*{Provides}?\\s*(\\{.*?\\})?(?=[\\:\\[\\]\\,\\{\\}\\.]|$)",
...
//      [jpql_macros.identifier, "return 'IDENTIFIER_NAME'"],
      ...
      ["{ActiveScriptExpressionToken}(\\:{ActiveScriptExpressionToken})?", "yytext = " + //piping of scripts would be implemented as script:script:script:script...
        "    {" +
      ...
      ["{ActiveFilterExpressionToken}(\\:{ActiveScriptExpressionToken})?", "yytext = " + //filter is passed an optional active branching teleportation device in the form os an activescribit
        "    {" +
        "      value: yy.lexer.matches[0]," +
        "      filter: {" +
        "        value: yy.lexer.matches[1]," +
        "        flag: yy.lexer.matches[2]," +
        "        tag: yy.lexer.matches[4]," +
        "        label: yy.lexer.matches[5]," +
        "        async: yy.lexer.matches[17]," +
        "        take: yy.lexer.matches[19]," +
        "        operation: yy.lexer.matches[20]," +
        "        provider: yy.lexer.matches[21]," +
        "        script: yy.lexer.matches[22]" +
        "      }," +
        "      stream: {" +
        "        value: yy.lexer.matches[24]," +
        "        tag: yy.lexer.matches[26]," +
        "        label: yy.lexer.matches[27]," +
        "        async: yy.lexer.matches[39]," +
        "        take: yy.lexer.matches[41]," +
        "        operation: yy.lexer.matches[42]," +
        "        provider: yy.lexer.matches[43]," +
        "        script: yy.lexer.matches[44]" +
        "      }" +
        "    }" +
        "; return 'ACTIVE_FILTER_EXPRESSION_TOKEN'"],
//      ["{RegularExpressionLiteral}(\\:{ActiveScriptExpressionToken})?", "yytext = yy.lexer.matches" + //regexp filter with mapper script
//        "    {" +
//        "      value: yy.lexer.matches[0]," +
//        "      filter: {" +
//        "        value: yy.lexer.matches[1]," +
...
//      ["{CallExpressionToken}", "return 'CALL_EXPRESSION_TOKEN'"],
...
//      [ 'CALL_EXPRESSION', '$$ = yy.ast.merge($1, { scope: "child" })' ]
...
//     [ 'END', '' ] ],
    ...
    SUBSCRIPT_EXPRESSION_NESTABLE: [
      ...
      /** second class listables don't work on the owner of the subscript expression, but anchor themselves at a position of one of the owners children */
    ...
    ACTIVE_ROOT: [
      [ 'ACTIVE_ROOT_TOKEN',                 '$$ = { expression: { type: "root|active", value:  String.fromCharCode(0x40) + "$" } }; yy.ast.node($$)' ]], //jison uses "@" internaly to reference current context
