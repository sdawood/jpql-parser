...
var grammar = {
  ...
  lex: {
    ...
    macros: {
      ...
      /** new macros from https://github.com/cjihrig/jsparser/blob/master/ecmascript.jison */
      Take: "#(\\(({DecimalIntegerLiteral})?\\))?",
      ReservedKeywords: jpql_macros.tokens.reservedWords.reservedKeywords,
      ActiveScriptExpressionComponentToken: "(({Take})?[~=\\+\\-\\*])?(\\{.+?\\})",
      ActiveScriptExpressionToken: "{ActiveScriptExpressionComponentToken}|{AsyncActiveScriptExpressionComponentToken}",
//      CallExpressionToken: "\\((?!\\s+){Identifier}\\:\\s*(.+?)\\s*\\)(?=[\\[\\]\\,\\{\\}\\.]|$)", //no spaces allowed between the opening "(" and the key, also for good style no spaces are allowed between key and ":"
      ScriptExpressionToken: "\\(.+?\\)(?=[\\:\\[\\]\\,\\{\\}\\.]|$)",
      FilterExpressionToken: "\\?(\\-)?\\(.+?\\)(?=[\\:\\[\\]\\,\\{\\}\\.]|$)",
      AsyncFilterExpressionToken: "\\?({Take})\\(.+?\\)(?=[\\:\\[\\]\\,\\{\\}\\.]|$)"
    },
...
//      [jpql_macros.identifier, "return 'IDENTIFIER_NAME'"],
      ...
      ["\\({ActiveScriptExpressionComponentToken}(\\:({ActiveScriptExpressionComponentToken}))?\\)(?=[\\:\\[\\]\\,\\{\\}\\.]|$)", "yytext = yy.lexer.matches; return 'SCRIPT_EXPRESSION_TOKEN'"],
