  ...
  lex: {
    ...
    macros: {
      ...
      /** new macros from https://github.com/cjihrig/jsparser/blob/master/ecmascript.jison */
        //*Take: "(#|@)"
      Tag: "(#){Identifier}?",
      AsyncTake: "(@)(\\({DecimalIntegerLiteral}?\\))?",
      Operation: "=>|[~=\\+&\\-\\*]", //map, (post or put), put, post, merge, delete, splat
      ReservedKeywords: jpql_macros.tokens.reservedWords.reservedKeywords,
      ActiveScriptExpressionToken: "\\(\\s*{Tag}?\\s*{AsyncTake}?\\s*{Operation}?\\s*(\\{.*?\\})?\\)(?=[\\:\\[\\]\\,\\{\\}\\.]|$)",
...
//      [jpql_macros.identifier, "return 'IDENTIFIER_NAME'"],
      ...
      ["{ActiveScriptExpressionToken}(\\:{ActiveScriptExpressionToken})?", "yytext = " +
        ...
        "      }," +
        "      reduce: {" +
        "        value: yy.lexer.matches[26]," +
        "        tag: yy.lexer.matches[28]," +
        "        label: yy.lexer.matches[29]," +
        "        async: yy.lexer.matches[41]," +
        "        take: yy.lexer.matches[43]," +
